    ...
    if (width < 0 || height < 0) {
        ...
        switch (dimensionsConstraint) {
            case DimensionsConstraint::POWER_OF_TWO_SQUARE:
                dimensions = packRectangles<SquarePowerOfTwoSizeSelector>(rectangles.data(), (int)rectangles.size(), spacing);
                break;
            case DimensionsConstraint::POWER_OF_TWO_RECTANGLE:
                dimensions = packRectangles<PowerOfTwoSizeSelector>(rectangles.data(), (int)rectangles.size(), spacing);
                break;
            case DimensionsConstraint::MULTIPLE_OF_FOUR_SQUARE:
                dimensions = packRectangles<SquareSizeSelector<4> >(rectangles.data(), (int)rectangles.size(), spacing);
                break;
            case DimensionsConstraint::EVEN_SQUARE:
                dimensions = packRectangles<SquareSizeSelector<2> >(rectangles.data(), (int)rectangles.size(), spacing);
                break;
            case DimensionsConstraint::SQUARE:
            default:
                dimensions = packRectangles<SquareSizeSelector<> >(rectangles.data(), (int)rectangles.size(), spacing);
                break;
        }
        ...
        if (int result = packRectangles(rectangles.data(), (int)rectangles.size(), width, height, spacing))
            return result;
    }
